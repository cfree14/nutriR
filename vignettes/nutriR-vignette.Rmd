...
In this vignette, we're going to compare habitual iron intakes in the USA and Bangladesh and calculate the magnitude of diet shifts required to reduce iron deficiencies in both locations. To begin, we'll use the get_dists function to retrieve habitual intake distributions for both countries.
...
We can also measure the similarity between the two distributions as the percent overlap between the distribution. In this case, we calculate percent overlap using the equations for the Bhattacharyya coefficient.
...
## 5. Quantifying nutrient deficiencies

We can use Estimated Average Requirements (EARs) and the probability method to calculate the nutrient deficiency associated with each of these distributions. This measure of nutrient deficiency is known as the summary exposure value (SEV).
...
The EAR for iron for 15-19-yr-old women is 7.9. We can calculate and compare iron deficiencies in the USA and Bangladesh using the sev() function as follows:
